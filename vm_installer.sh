            ...
            case $edit_choice in
                1)
                    while true; do
                        ...
                        if validate_input "name" "$new_hostname"; then HOSTNAME="$new_hostname"; break; fi
                    done
                    ;;
                2)
                    while true; do
                        ...
                        if validate_input "username" "$new_username"; then USERNAME="$new_username"; break; fi
                    done
                    ;;
                3)
                    while true; do
                        ...
                        echo
                        if [ -n "$new_password" ]; then PASSWORD="$new_password"; break
                        else print_status "ERROR" "Password cannot be empty"; fi
                    done
                    ;;
                4)
                ...
                7)
                    while true; do
                        read -p "$(print_status "INPUT" "Memory in MB (current: $MEMORY): ")" new_memory
                        new_memory="${new_memory:-$MEMORY}"
                        if validate_input "number" "$new_memory"; then MEMORY="$new_memory"; break; fi
                    done
                    ;;
                8)
                    while true; do
                        read -p "$(print_status "INPUT" "CPU count (current: $CPUS): ")" new_cpus
                        new_cpus="${new_cpus:-$CPUS}"
                        if validate_input "number" "$new_cpus"; then CPUS="$new_cpus"; break; fi
                    done
                    ;;
                9)
                    while true; do
                        ...
                        if validate_input "size" "$new_disk_size"; then DISK_SIZE="$new_disk_size"; break; fi
                    done
                    ;;
                0) return 0 ;;
        ...
        while true; do
            ...
            if validate_input "size" "$new_disk_size"; then
                if [[ "$new_disk_size" == "$DISK_SIZE" ]]; then
                    print_status "INFO" "Same size. No changes made."
                    return 0
                fi
                ...
                [[ "$current_unit" =~ [Gg] ]] && current_size_num=$((current_size_num * 1024))
                [[ "$new_unit" =~ [Gg] ]] && new_size_num=$((new_size_num * 1024))
        ...
        case $choice in
            ...
            2)
                if [ $vm_count -gt 0 ]; then
                    ...
                    else print_status "ERROR" "Invalid selection"; fi
                fi ;;
            3)
                if [ $vm_count -gt 0 ]; then
...
# ✅ ALL 8 OS OPTIONS (matching original)
